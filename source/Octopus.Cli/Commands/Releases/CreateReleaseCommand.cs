...
namespace Octopus.Cli.Commands.Releases
{
    ...
    public class CreateReleaseCommand : DeploymentCommandBase, ISupportFormattedOutput
    {
            ...
            : base(repositoryFactory, fileSystem, clientFactory, commandOutputProvider)
        {
            ...
            options.Add("packageprereleasefallbacks=", "[Optional] Semicolon-separated pre-release fallback tags, evaluate sequentally until first match. In case of regular packageprerelease does not match.", v => versionPreReleaseTagFallBacks = v);
        ...
        private async Task<ReleasePlan> BuildReleasePlan(ProjectResource project)
        {
            if (!string.IsNullOrWhiteSpace(ChannelNameOrId))
            {
                ...
                return await releasePlanBuilder.Build(Repository, project, matchingChannel, VersionPreReleaseTag, versionPreReleaseTagFallBacks).ConfigureAwait(false);
            }
            ...
            return await releasePlanBuilder.Build(Repository, project, null, VersionPreReleaseTag, versionPreReleaseTagFallBacks).ConfigureAwait(false);
        }
        ...
        async Task<ReleasePlan> AutoSelectBestReleasePlanOrThrow(ProjectResource project)
        {
            ...
            foreach (var channel in candidateChannels)
            {
                ...
                var plan = await releasePlanBuilder.Build(Repository, project, channel, VersionPreReleaseTag, versionPreReleaseTagFallBacks).ConfigureAwait(false);
